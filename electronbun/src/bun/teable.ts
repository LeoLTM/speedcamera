...
// Self-hosted Teable instances frequently have incomplete certificate chains.
// Bun's fetch supports a `tls` extension to opt out of strict CA verification.
...
export async function testTeableConnection(url: string, token: string): Promise<TeableUser> {
  ...
  // Step 1: Verify token via GET /api/space
  ...
  // Step 2: Try to get user profile (may fail with scoped PATs — that's fine)
  ...
  try {
    ...
    // Fallback — connection is still valid
  }
