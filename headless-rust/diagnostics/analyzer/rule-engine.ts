...
export function evaluateDiagnosticFindings(results: {
    ...
    findings.push({
      ...
      remediation: {
        ...
        commands: [
          ...
          "# 3. Optimize NetworkManager Wi-Fi AP settings to keep connection active",
          "sudo nmcli connection modify 'Speedcamera-Hotspot' 802-11-wireless.powersave 2",
        ],
      },
    });
  }
  ...
  if (bufferConfidence >= 40) {
    findings.push({
      ...
      remediation: {
        description:
          "Keep GigE camera buffer maximums (rmem_max / wmem_max = 128MB) so camera capture sockets can allocate large buffers explicitly via SO_RCVBUF, but restore default socket buffer sizes (rmem_default / wmem_default = 212KB) for general TCP/Wi-Fi sockets.",
        commands: [
          "# Restore sensible default socket buffers while keeping max limits for GigE camera",
