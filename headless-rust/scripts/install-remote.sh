...
for arg in "$@"; do
  case "$arg" in
    ...
    --help|-h)
      ...
      echo "  • On Pi 5, the release binary is compiled natively or synced directly."
      exit 0
      ;;
    *)
      if [[ "$arg" != -* ]]; then
...
REMOTE_CMD="chmod +x ~/speedcamera/headless-rust/scripts/*.sh && ~/speedcamera/headless-rust/scripts/install-rpi.sh ${NETWORK_FLAG}"
...
if [ "$NETWORK_FLAG" = "--field" ]; then
  ...
  echo "   • Switch to field mode later: ssh ${PI_TARGET} '~/speedcamera/headless-rust/scripts/setup-network.sh field'"
fi
echo ""
