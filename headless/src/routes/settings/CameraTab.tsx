...
import type { HwControl, SystemNetworkSummary } from "@/shared/types";
    ...
    <div className="space-y-3 rounded-lg border border-border p-4 bg-muted/20">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
        ...
        <div className="p-3 rounded-md border border-border/70 bg-card space-y-1.5">
          <div className="flex items-center justify-between">
            <span className="font-semibold text-foreground">Camera LAN (eth0)</span>
            {network.cameraLan.status === "ok" ? (
              <Badge variant="outline" className="text-green-500 border-green-500/30 text-[10px] py-0">192.168.1.100 OK</Badge>
            ) : network.cameraLan.status === "ip_mismatch" ? (
              <Badge variant="destructive" className="text-[10px] py-0">{network.cameraLan.ip || "Mismatch"}</Badge>
            ) : (
              <Badge variant="secondary" className="text-[10px] py-0">Not Found</Badge>
            )}
          </div>
          <div className="text-[11px] text-muted-foreground space-y-0.5 font-mono">
            <div>IP: {network.cameraLan.ip || "Unassigned"}</div>
            <div>Subnet: {network.cameraLan.subnet}</div>
          </div>
          <p className="text-[10px] text-muted-foreground/80 leading-tight pt-1 border-t border-border/40">
            Dedicated interface for GigE Vision camera streaming.
          </p>
        </div>
        ...
        <div className="p-3 rounded-md border border-border/70 bg-card space-y-1.5">
          <div className="flex items-center justify-between">
            <span className="font-semibold text-foreground">Hotspot AP (wlan0)</span>
            {network.hotspotAp.status === "ok" ? (
              <Badge variant="outline" className="text-green-500 border-green-500/30 text-[10px] py-0">192.168.4.1 OK</Badge>
            ) : network.hotspotAp.status === "ip_mismatch" ? (
              <Badge variant="outline" className="text-blue-500 border-blue-500/30 text-[10px] py-0">{network.hotspotAp.ip}</Badge>
            ) : (
              <Badge variant="secondary" className="text-[10px] py-0">Inactive</Badge>
            )}
          </div>
          <div className="text-[11px] text-muted-foreground space-y-0.5 font-mono">
            <div>IP: {network.hotspotAp.ip || "Unassigned"}</div>
            <div>Web UI: :3000</div>
          </div>
          <p className="text-[10px] text-muted-foreground/80 leading-tight pt-1 border-t border-border/40">
